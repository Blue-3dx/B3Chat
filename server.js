...
case 'checkAdmin':
  db.get("SELECT admin FROM users WHERE username = ?", [data.username], (err, row) => {
    if (err) return console.error(err); // You can log this if you want
    if (row) {
      socket.send(JSON.stringify({ type: 'adminStatus', isAdmin: row.admin === 1 }));
    } else {
      socket.send(JSON.stringify({ type: 'adminStatus', isAdmin: false }));
    }
  });
  break;
